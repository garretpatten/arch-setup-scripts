...
if ! command -v pass-cli >/dev/null 2>&1; then
    ...
    if curl -fsSL --retry 3 --retry-delay 2 "$proton_pass_cli_url" -o "$proton_pass_cli" 2>/dev/null; then
        if [[ -s "$proton_pass_cli" ]] && [[ "$(head -c 4 "$proton_pass_cli" 2>/dev/null)" == $'\x7fELF' ]]; then
            ...
            sudo install -m 755 "$proton_pass_cli" /usr/local/bin/pass-cli 2>/dev/null || true
        fi
    fi
fi

# Arch also uses the legacy 'protonpass' binary name in some validation flows.
if ! command -v protonpass >/dev/null 2>&1 && command -v pass-cli >/dev/null 2>&1; then
    sudo ln -sf "$(command -v pass-cli)" /usr/local/bin/protonpass 2>/dev/null || true
fi
