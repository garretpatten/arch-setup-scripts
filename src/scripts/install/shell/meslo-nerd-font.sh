...
curl -fsSL https://github.com/ryanoasis/nerd-fonts/releases/download/v3.4.0/Meslo.zip -o "$meslo_zip" 2>/dev/null || exit 0
sudo mkdir -p "$font_dir"
unzip -q "$meslo_zip" -d "$temp_font_dir" 2>/dev/null || true
